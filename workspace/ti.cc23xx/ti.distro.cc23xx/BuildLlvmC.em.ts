...
export function em$generate() {
    ...
    out.addFrag(`
        |-> #!/bin/sh
        |-> 
        |-> INST=/c/tools/LLVM-ET-Arm-19.1.5-Windows-x86_64
        |-> 
        |-> CC=$INST/bin/clang
        |-> LD=$INST/bin/ld.lld
        |-> OBJCOPY=$INST/bin/llvm-objcopy
        |-> OBJDUMP=$INST/bin/llvm-objdump
        |-> 
        |-> OUT=.out
        |-> 
        |-> rm -rf $OUT
        ...
        |-> 
        |-> LIBS="\\
        |->     $INST/lib/clang-runtimes/arm-none-eabi/armv6m_soft_nofp/lib/libclang_rt.builtins.a \\
        |->     $INST/lib/clang-runtimes/arm-none-eabi/armv6m_soft_nofp/lib/libc.a \\
        |-> "
        |-> 
