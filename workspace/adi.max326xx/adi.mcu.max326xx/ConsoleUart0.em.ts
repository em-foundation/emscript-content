...
export function em$startup() {
    e$`MXC_GCR->pclkdis0 &= ~(MXC_F_GCR_PCLKDIS0_UART0)`
    TxPin.$$.makeOutput()
    TxPin.$$.functionSelect(2)
    e$`MXC_GCR->clkctrl |= MXC_F_GCR_CLKCTRL_IBRO_EN`
    e$`MXC_SETFIELD(MXC_UART0->ctrl, MXC_F_UART_CTRL_CHAR_SIZE, MXC_S_UART_CTRL_CHAR_SIZE_8BITS)`
    e$`MXC_UART0->osr = 5`
    const cd = clkdiv.$$
    e$`MXC_UART0->clkdiv = cd`
    e$`MXC_UART0->ctrl |= (MXC_S_UART_CTRL_BCLKSRC_PERIPHERAL_CLOCK | MXC_F_UART_CTRL_BCLKEN | MXC_F_UART_CTRL_UCAGM)`
}

export function flush(): void {
    while ((e$`MXC_UART0->status & MXC_F_UART_STATUS_TX_EM`) == 0) {}
}

export function put(data: u8): void {
    e$`MXC_UART0->fifo = data`
    flush()
}
